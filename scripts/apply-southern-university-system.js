...
const VALIDATION_PATH = path.join(ROOT, "generated", "southern-university-system-validation.json");
const REPORT_PATH = path.join(ROOT, "generated", "southern-university-system-milestone.json");
const SOURCE = "Southern University System";
...
for (const control of validation.results) {
  const scope = control.platformType === "southern-vacancies"
    ? "its institution-owned SUNO vacancy feed"
    : "the shared system feed with exact SUBR/SULC/SU SYSTEM title-prefix routing";
  const notes = `Verified live 2026-08-25 from ${scope}. Production validation used two total scans for four institutions and found ${control.currentFacultyJobCount} current faculty jobs for this institution; ambiguous and unprefixed shared-feed rows are rejected.`;
  const override = {
    name: control.name,
  ...
  institution.last_discovery_status = "official_exact_institution_scope_validated";
...
const report = {
  generatedAt: now,
  scope: "Southern University System exact institution routing",
  baseline: {
  ...
  safeguards: [
    "The shared Southern System feed is scanned once for three institutions.",
    "Only exact SUBR, SULC, SU SYSTEM, or SUS title prefixes can assign a shared-feed row.",
    "Unprefixed, SUAREC, SUNO, and SUSLA shared-feed rows fail closed.",
    "Explicitly expired application deadlines are rejected.",
...
console.log(`Applied ${validation.results.length} Southern University controls; ${newlyCoveredCount} newly covered.`);
