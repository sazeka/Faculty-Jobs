...
const validation = read("generated/first-remaining-batch-validation.json");
...
for (const control of validation.promoted) {
  const notes = `Verified in the first remaining-coverage batch on 2026-08-27. ${control.evidence}`;
  const replacement = {
    name: control.name,
...
write("generated/first-remaining-batch-milestone.json", {
