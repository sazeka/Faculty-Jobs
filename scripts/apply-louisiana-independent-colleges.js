...
const VALIDATION_PATH = path.join(ROOT, "generated", "louisiana-independent-colleges-validation.json");
const REPORT_PATH = path.join(ROOT, "generated", "louisiana-independent-colleges-milestone.json");
...
for (const control of validation.results) {
  const availability = control.explicitNoOpenings
    ? "The page explicitly states that the institution is not currently hiring."
    : `Production validation found ${control.currentFacultyJobCount} current faculty jobs.`;
  const notes = `Verified live 2026-08-25 from the institution's official employee hiring page. ${availability} Institution-specific source health was confirmed, and navigation-only faculty labels are rejected.`;
  const override = {
    name: control.name,
...
const report = {
  generatedAt: now,
  scope: "Four Louisiana independent-college official hiring pages",
  baseline: {
  ...
  safeguards: [
    "Every source is an institution-owned employee hiring page or dedicated ATS host.",
    "Xavier is fixed to its PeopleAdmin Faculty position type.",
    "Saint Joseph's navigation-only Faculty Members label is explicitly rejected.",
    "A healthy official source remains coverage evidence when it explicitly has zero current openings.",
...
console.log(`Applied ${validation.results.length} Louisiana independent controls; ${newlyCoveredCount} newly covered.`);
