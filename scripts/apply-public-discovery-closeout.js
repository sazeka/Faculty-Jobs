...
const validation = read("generated/public-discovery-closeout-validation.json");
...
for (const control of validation.promoted) {
  const notes = `Verified in the public-institution discovery closeout on 2026-08-27. ${control.evidence}`;
...
write("generated/public-discovery-closeout-milestone.json", {
  ...
  unreviewedEligibleAfter: 0,
  safeguards: validation.safeguards
});

console.log(`Applied ${validation.promotedCount} source; ${master.counts.covered - coveredBefore} newly covered; ${validation.heldCount} held.`);
