// One-off retroactive re-enrich. Re-runs the local model on rows whose
// positionType is "Other" (likely degraded from an off-enum string the old
// normalizeResult dropped) plus any never-enriched rows, then applies the new
// coercePositionType / coerceTenureTrack mapping. Writes public/jobs.json and
// docs/jobs.json; run rebuild-data-chunks-all.js afterward to reach the chunks.
//
// Safe by design: positionType + tenureTrack are always updated from the fresh
// result; discipline is only overwritten when the new value is non-null (so we
// never lose an existing discipline to a null re-classification).
const fs = require('fs');
...
async function main() {
  ...
  for (let b = 0; b < totalBatches; b++) {
    ...
    if ((b + 1) % SAVE_EVERY === 0 || b === totalBatches - 1) {
      save(payload);
      console.log(`batch ${b + 1}/${totalBatches} | items ${stats.items} | positionType recovered ${stats.ptRecovered} | tenureTrack recovered ${stats.ttRecovered} | dropped ${stats.dropped} | errors ${stats.errors} (saved)`);
    }
  }
