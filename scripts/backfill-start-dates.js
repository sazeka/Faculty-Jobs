...
let filled = 0, ymd = 0, season = 0, skippedStale = 0;
const samples = [];
for (const job of payload.jobs) {
  if (job.startDate || !job.description) continue;
  const sd = extractStartDate(job.description);
  if (!sd) continue;
  // A recycled/stale posting's "Desired Start Date" can predate when the
  // listing was actually (re-)posted -- don't backfill an impossible
  // "Anticipated start" (issue #156).
  if (isImplausibleStartDate(sd, job.datePosted)) { skippedStale++; continue; }
  job.startDate = sd;
  filled++;
...
console.log(`Filled startDate on ${filled} jobs (${ymd} calendar dates, ${season} season/month; ${skippedStale} skipped as implausible/stale).`);
