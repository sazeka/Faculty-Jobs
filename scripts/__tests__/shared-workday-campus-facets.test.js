...
test("Penn State New Kensington remains unpromoted without a verified facet", () => {
  ...
  assert.ok(["missing", "excluded_policy"].includes(master.institutions.find((row) => row.name === name)?.coverage_status));
  assert.equal(rules.institutionOverrides[name]?.action, "exclude");
