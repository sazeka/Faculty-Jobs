...
const CLEARLY_NON_TENURE_TITLE_RE = /\badj\.\s+(?:faculty|instructor|lecturer|professor)\b|\b(?:adjunct|visiting|post[\s-]?doc(?:toral)?|fellow|temporary|limited[\s-]?term|part\s*-?\s*time|half\s*-?\s*time|professor\s+of\s+practice|(?:assistant|associate|full)?\s*professor[\s-]+in[\s-]+residence|wot|affiliate\s+(?:faculty|instructor|lecturer|professor)|per\s+diem|non[\s-]?compensated|contract(?:ual)?\s+(?:faculty|instructor|lecturer)|acting\s+(?:assistant|associate|full)?\s*professor|substitute\b.{0,45}\binstructors?|restricted(?:\s+\d+[\s-]month)?\s+faculty|on[\s-]call|hourly\b.{0,35}\b(?:faculty|instructor|lecturer)|non[\s-]?credit\b.{0,35}\binstructors?)\b|\binstructors?\b.{0,35}\bnon[\s-]?credit\b|\bterm(?:\s*[-,]\s*|\s+)(?:assistant|associate|full)?\s*(?:professor|faculty|lecturer|instructor)\b|\b(?:assistant|associate|full)?\s*(?:professor|faculty|lecturer|instructor)\b(?:\s+\d+)?(?:\s*[-,]\s*|\s+)term\b|\b(?:instructors?|lecturers?)\b.{0,45}\b(?:applicant\s+)?pool(?:\s+posting)?\b|\bpool\s+posting\b.{0,45}\b(?:instructors?|lecturers?)\b/i;
