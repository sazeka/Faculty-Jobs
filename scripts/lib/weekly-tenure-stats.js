...
const NON_TENURE_RE = /\b(?:non[\s-]?tenured?(?:[\s-]?(?:track|accru(?:ing|al)|eligible))?|non[\s-]?tenurable|without\s+tenure|not\s+(?:a\s+)?tenure[\s-]?(?:track|eligible|accruing)|not\s+eligible\s+for\s+tenure|ineligible\s+for\s+tenure|outside\s+(?:the\s+)?tenure\s+structure|ntt|teaching[\s-]?track|instructional[\s-]?track|professional[\s-]?track|practice[\s-]?track|clinical[\s-]?track|research[\s-]?track|career[\s-]?track\s+(?:(?:faculty|teaching)\s+)?(?:appointment|position)|fixed[\s-]?term|temporary\s+replacement|limited[\s-]?term\s+(?:faculty\s+)?(?:appointment|position)|time[\s-]?limited\s+(?:faculty\s+)?(?:appointment|position)(?!\s*:?\s*no\b)|renewable\s+term(?:\s+(?:appointment|position))?|(?:one|two|three|1|2|3)[\s-]year\s+(?:(?:faculty|academic)\s+)?(?:appointment|position|term)|term[\s-]?faculty|contingent\s+(?:faculty|appointment|position|employee|status|worker))\b/i;
