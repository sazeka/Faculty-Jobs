...
const NON_TENURE_RE = /\b(?:non[\s-]?tenured?(?:[\s-]?(?:track|accru(?:ing|al)|eligible))?|non[\s-]?tenurable|without\s+tenure|not\s+(?:a\s+)?tenure[\s-]?(?:track|eligible|accruing)|not\s+eligible\s+for\s+tenure|ineligible\s+for\s+tenure|outside\s+(?:the\s+)?tenure\s+structure|ntt|teaching[\s-]?track|instructional[\s-]?track|professional[\s-]?track|practice[\s-]?track|clinical[\s-]?track|research[\s-]?track|career[\s-]?track\s+(?:(?:faculty|teaching)\s+)?(?:appointment|position)|fixed[\s-]?term|temporary\s+replacement|limited[\s-]?term\s+(?:faculty\s+)?(?:appointment|position)|time[\s-]?limited\s+(?:faculty\s+)?(?:appointment|position)(?!\s*:?\s*no\b)|renewable\s+term(?:\s+(?:appointment|position))?|(?:one|two|three|1|2|3)[\s-]year\s+(?:(?:faculty|academic)\s+)?(?:appointment|position|term)|(?:initial\s+)?appointment\s+is\s+for\s+a\s+term\s+of\s+up\s+to\s+(?:one|two|three|\d+)\s+years?|term[\s-]?faculty|contingent\s+(?:faculty|appointment|position|employee|status|worker))\b/i;
...
const CLEARLY_NON_TENURE_TITLE_RE = /\badj\.\s+(?:faculty|instructor|lecturer|professor)\b|\b(?:adjunct|visiting|post[\s-]?doc(?:toral)?|fellow|temporary|limited[\s-]?term|part\s*-?\s*time|professor\s+of\s+practice|(?:assistant|associate|full)?\s*professor[\s-]+in[\s-]+residence|wot|affiliate\s+(?:faculty|instructor|lecturer|professor)|per\s+diem|non[\s-]?compensated|contract(?:ual)?\s+(?:faculty|instructor|lecturer)|acting\s+(?:assistant|associate|full)?\s*professor|substitute\b.{0,45}\binstructors?|restricted(?:\s+\d+[\s-]month)?\s+faculty|on[\s-]call|hourly\b.{0,35}\b(?:faculty|instructor|lecturer)|non[\s-]?credit\b.{0,35}\binstructors?)\b|\binstructors?\b.{0,35}\bnon[\s-]?credit\b|\bterm(?:\s*[-,]\s*|\s+)(?:assistant|associate|full)?\s*(?:professor|faculty|lecturer|instructor)\b|\b(?:assistant|associate|full)?\s*(?:professor|faculty|lecturer|instructor)\b(?:\s+\d+)?(?:\s*[-,]\s*|\s+)term\b|\b(?:instructors?|lecturers?)\b.{0,45}\b(?:applicant\s+)?pool(?:\s+posting)?\b|\bpool\s+posting\b.{0,45}\b(?:instructors?|lecturers?)\b/i;
