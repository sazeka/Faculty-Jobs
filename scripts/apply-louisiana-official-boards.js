...
const VALIDATION_PATH = path.join(ROOT, "generated", "louisiana-official-boards-validation.json");
const REPORT_PATH = path.join(ROOT, "generated", "louisiana-official-boards-milestone.json");
...
for (const control of validation.results) {
  const notes = `Verified live 2026-08-25 from the institution's official careers page and dedicated ${control.platformType === "workday" ? "Workday tenant" : "PeopleAdmin board"}. Production validation found ${control.currentFacultyJobCount} current faculty jobs and no cross-institution rows.`;
  const override = {
    name: control.name,
  ...
  institution.last_discovery_status = "official_dedicated_board_validated";
...
const report = {
  generatedAt: now,
  scope: "Nicholls and University of New Orleans dedicated official boards",
  baseline: {
  ...
  safeguards: [
    "Both sources are linked from official institution HR or careers pages.",
    "Each ATS hostname or tenant is dedicated to one institution.",
    "Production faculty-title filters remain active.",
...
console.log(`Applied ${validation.results.length} Louisiana official boards; ${newlyCoveredCount} newly covered.`);
